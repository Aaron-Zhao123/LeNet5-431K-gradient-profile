...
def recover_weights(weights_mask, wegihts, grads, recover_rates):
    ...
    for key in keys:
        mean_w = np.mean(wegihts)
        std_w = np.std(wegihts)
        mean_g = np.mean(grads)
        std_g = np.std(grads)
        # if (key == 'fc1'):
        #     print('mask grads, mean {}, std {}'.format(mean_grad,std_grad))
        mask_w = np.abs(weights[key]) > (mean_w + recover_rates[key] * std_w)
        mask_g = np.abs(grads[key]) > (mean_g + recover_rates[key] * std_g)
        mask_w.astype(int)
        mask_g.astype(int)
    ...
    try:
        ...
        with tf.Session() as sess:
            ...
            if (PROFILE == True):
                ...
                weights_mask = recover_weights(weights_mask, pre_profile_weights, collect_grads, recover_rates)
