...
def initialize_variables(parent_dir, model_number, weights_mask, rmask, profile = False, train = False, prune = False):
...
def recover_weights(weights_mask, grad_probs, recover_rates):
    ...
    for key in keys:
        if (recover_rates[index] == 0):
            ...
            print(perc_bar)
            threshold = np.percentile(grad_probs,perc_bar)
        index += 1
